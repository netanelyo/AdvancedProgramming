...
void Game::checkIfBoardIsValid(int shouldPrintErrMsg[], std::map<char, int>& shipsErrorMsgMap)
{
	...
	for (auto i = 0; i < GameConstants::BOARD_SIZE; ++i)
	{
		for (auto j = 0; j < GameConstants::BOARD_SIZE; ++j)
		{
			...
			/*initializes variables for next iteration*/
			shipLength = 1;
			invalidShape = 0;
			adjShips = 0;
		}
...
{
	/*marks the sunk ship with @, and prints it to screen if necessary*/
	m_gameBoard[row][col] = '@';
	if (!m_quiet) GameDisplayUtils::printSquare(col, row, '@', Color::RED);
	/*finds all the other squares belonging to the sunk ships in order to mark them as well*/
...
GameState Game::playMove()
{
	...
	/*checks if both players got no more moves and if so -> game is over*/
	if (endOfAttacks())
	{
		return GameState::GAME_OVER;
	}

	/*if only the current player got no more moves, it means that we got no attack and we only pass the turn to the opponent*/
	...
	/*the attack result was a miss*/
	if (square == '0' || square == '@')
	{
		result = AttackResult::Miss;
		m_nextPlayer ^= (canPass ? 1 : 0); /* If canPassTurn -> nextPlayer changes */
		/*marks the relevant square as a miss (if it wasn't due to a sunk ship) 
		using the color blue if the attacker was player A or green if the attacker was player B*/
		if (!m_quiet && square != '@') GameDisplayUtils::printSquare(colCoord, rowCoord, '*',
			SET_COLOR(currentPlayer));
	}
	/*the attack result was a hit or a sink*/
	else
	{
		...
		/*marks the relevant square as a hit (if it was a sink we later mark the entire ship with @)
		using the color blue if the attacker was player A or green if the attacker was player B*/
...
AttackResult Game::determineAttackResult(char square, int rowCoord, int colCoord) const
{
	...
	while (row < GameConstants::BOARD_SIZE - (len - tmp) && tmp > 0)
	{
		CHECK_CURRENT_SQUARE(rowCoord + (len - tmp), colCoord);
	}
	...
	while (col < GameConstants::BOARD_SIZE - (len - tmp) && tmp > 0)
	{
		CHECK_CURRENT_SQUARE(rowCoord, colCoord + (len - tmp));
	}
